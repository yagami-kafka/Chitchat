...
class DiffieHellman:
    ...
    def generate_shared_key(self, other_key_str: str) -> str:
        ...
        second_generator = random.randint(2,shared_key-1)
        public_key_second = self.second_public_key(second_generator, shared_key)
        final_shared = self.second_shared_key(public_key_second,shared_key)
        return sha256(str(final_shared).encode()).hexdigest()
        # return sha256(str(shared_key).encode()).hexdigest()

    def second_public_key(self, second_generator, first_shared_key) -> str:
        second_public_key = pow(second_generator, self.__second_private_key, first_shared_key)
        return hex(second_public_key)[2:]
    
    def second_shared_key(self, second_pub_key_str: str, first_shared_key) ->str:
        second_pub_key = int(second_pub_key_str , base = 16)
        second_shared = pow(second_pub_key, self.__second_private_key, first_shared_key)
